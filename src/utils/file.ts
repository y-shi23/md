...
function getConfig(useDefault: boolean, platform: string) {
  ...
  // load configuration from utools.dbStorage
  const customConfig = JSON.parse(utools.dbStorage.getItem(`${platform}Config`)!)
...
async function ghFileUpload(content: string, filename: string) {
  const useDefault = utools.dbStorage.getItem(`imgHost`) === `default`
...
async function qiniuUpload(file: File) {
  const { accessKey, secretKey, bucket, region, path, domain } = JSON.parse(
    utools.dbStorage.getItem(`qiniuConfig`)!,
  )
...
async function txCOSFileUpload(file: File) {
  ...
  const { secretId, secretKey, bucket, region, path, cdnHost } = JSON.parse(
    utools.dbStorage.getItem(`txCOSConfig`)!,
  )
  const cos = new COS({
    SecretId: secretId,
    SecretKey: secretKey,
...
async function minioFileUpload(file: File) {
  ...
  const { endpoint, port, useSSL, bucket, accessKey, secretKey } = JSON.parse(
    utools.dbStorage.getItem(`minioConfig`)!,
  )
...
async function getMpToken(appID: string, appsecret: string, proxyOrigin: string) {
  const data = utools.dbStorage.getItem(`mpToken:${appID}`)
  if (data) {
  ...
  if (res.access_token) {
    ...
    utools.dbStorage.setItem(`mpToken:${appID}`, JSON.stringify(tokenInfo))
...
async function mpFileUpload(file: File) {
  let { appID, appsecret, proxyOrigin } = JSON.parse(
    utools.dbStorage.getItem(`mpConfig`)!,
  )
  // 未填写代理域名且是cfpages环境
...
async function r2Upload(file: File) {
  const { accountId, accessKey, secretKey, bucket, path, domain } = JSON.parse(
    utools.dbStorage.getItem(`r2Config`)!,
  )
  const dir = path ? `${path}/` : ``
...
async function upyunUpload(file: File) {
  const { bucket, operator, password, path, domain } = JSON.parse(
    utools.dbStorage.getItem(`upyunConfig`)!,
  )
...
async function telegramUpload(file: File): Promise<string> {
  const { token, chatId } = JSON.parse(utools.dbStorage.getItem(`telegramConfig`)!)
...
/**
 * utools.dbStorage 中 cloudinaryConfig 的示例：
 * {
 *   "cloudName": "demo",
 *   "apiKey": "1234567890",
...
async function formCustomUpload(content: string, file: File) {
  const str = `
    async (CUSTOM_ARG) => {
      ${utools.dbStorage.getItem(`formCustomConfig`)}
    }
  `
  return new Promise<string>((resolve, reject) => {
    const exportObj = {
...
export function fileUpload(content: string, file: File) {
  const imgHost = utools.dbStorage.getItem(`imgHost`)
  if (!imgHost) {
    utools.dbStorage.setItem(`imgHost`, `default`)
  }
  switch (imgHost) {
    case `aliOSS`:
